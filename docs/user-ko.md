...
설치 후 SMAPI로 게임을 실행했을 때 로드된 모드 목록에 `pumasi 0.1.17`가 보이면 정상입니다.
...
항목이 화면보다 많으면 마우스 휠이나 오른쪽 스크롤바로 아래 항목을 볼 수 있습니다. 상단 메뉴 탭 공간이 부족한 화면에서는 Pumasi 탭이 바닐라 탭과 겹치지 않도록 탭 줄 아래쪽으로 내려갑니다.
...
`pms_ask`와 `/pms`는 입력을 먼저 규칙으로 분류하고, 애매하면 Gemini가 최근 대화와 현재 투두를 보고 한 번 더 판단합니다.
...
- 품앗이 자체에 대한 질문이나 일반 대화 질문은 바로 위키 검색으로 보내지 않고 대화 맥락 라우팅으로 넘깁니다.
- 인사, 감사, 짧은 반응, `그거 해줘` 같은 맥락 의존 입력은 최근 대화와 투두를 보고 일반 답변, 작업 계획, 위키 답변 중 하나로 이어집니다.
- 그래도 안전하게 추론할 수 없을 때만 더 구체적으로 말해달라고 답합니다.
