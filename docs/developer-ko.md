...
- 멀티플레이 메시지 라우팅.
- ESC 게임 메뉴의 Pumasi 빠른 설정 탭.
- Gemini 계획과 위키 기반 답변 흐름.

`TaskManager`는 투두 큐를 소유하고, 보이는 투두 순서를 유지하며, 호스트의 활성 투두 순서 조정을 지원하고, task key로 활성 작업 중복을 방지합니다.

`Pumasi.Core.Ui.TodoDisplayFormatter`와 `TodoDisplayFilter`는 공통 투두 행 문구와 표시 규칙을 담당합니다. 투두 보드와 `/pms todo`가 모두 이 Core 로직을 사용하므로 활성 행, 최근 종료 행, 우선순위, 출처, 대상 타일, 지역화된 결과 이유가 일관되게 표시됩니다.
...
`Pumasi.Core.Chat.HelperChatFormatter`는 도우미 답변을 인게임 채팅 줄로 포매팅합니다. 위키 출처 메타데이터는 메시지 모델에 남을 수 있지만, 사용자 요청에 따라 채팅/콘솔에는 출처 줄을 표시하지 않습니다.
...
설정 항목 순서, 라벨, 짧은 설명은 `Pumasi.Core.Ui.PumasiSettingsCatalog`가 소유하며, `PumasiSettingsCatalogTests`가 항목 순서와 라벨/설명 존재를 검증합니다.

언어 설정은 `UiConfig.Language`에 저장됩니다. Pumasi 자체 UI 문구는 `Pumasi.Core.Ui.PumasiText`를 통해 한국어/영어를 선택하고, 설정 행 라벨과 설명은 `PumasiSettingsCatalog`가 같은 언어 설정을 사용해 포매팅합니다.
...
Gemini 작업 계획은 후보 작업에 잠겨 있습니다. `PlanWithGeminiAsync`는 현재 호스트 상태를 스캔해 후보 작업 key를 Gemini에 보내고, 모델이 돌려준 작업 중 스캔 후보 key와 일치하는 작업만 투두에 추가합니다. 실제 안전 경계는 이 코드 검증이며, 프롬프트 지시만 신뢰하지 않습니다.
...
- 스프링클러 주변 일반 땅 파기.
- 동물 건물 건초 리필.
- 로드된 동물 건물에서 아직 쓰다듬지 않은 동물 쓰다듬기.
- 로드된 동물 건물의 바닥 동물 생산품 수거.
...
- `Animals` 작업 카테고리가 켜진 경우 로드된 동물 건물의 아직 쓰다듬지 않은 동물 후보.
- `Animals` 작업 카테고리가 켜진 경우 로드된 동물 건물의 바닥 동물 생산품 후보.

동물 생산품 보관은 의도적으로 보수적입니다. `CollectAnimalProduct`는 먼저 같은 qualified item ID와 품질이 이미 들어 있는 로드된 일반 상자를 찾고, 그 상자가 전체 스택을 받을 수 있을 때만 보관합니다. 맞는 상자가 없으면 호스트 인벤토리에 넣고, 호스트 인벤토리도 받을 수 없으면 월드 오브젝트를 그대로 둔 채 작업을 건너뜁니다. 이 동작은 일반 상자 정리가 아니며 임의의 빈 상자를 사용하면 안 됩니다.

보상과 XP는 호스트 귀속입니다. 품앗이는 실제 멀티플레이 농장 일꾼이 아니므로, 코드에서 XP API를 직접 호출하거나 게스트에게 보상을 분배하지 않아야 합니다. 바닐라 상호작용이 부가 효과를 지급한다면 호스트 실행 경로의 `Game1.player`에게 귀속됩니다.

아직 구현되지 않은 범위:

- 전체 NPC 일정, 호감도, 선물, 이벤트, 사회적 행동.
- 씨앗 심기, 판매, 파괴, 희귀 아이템 이동, 우유 짜기/양털 깎기 같은 도구 필요 동물 생산품 수거, 일반 상자 관리.
