# pumasi (품앗이) NPC Design

## Purpose

Create pumasi (`pms`, Korean name "품앗이"), a SMAPI mod for Stardew Valley multiplayer where one shared helper NPC assists with repetitive farm work. The helper should feel like a single visible character to all players, expose an in-game todo list, and use AI for planning, prioritization, and natural-language interaction.
